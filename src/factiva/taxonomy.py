...
def get_category_codes(taxonomy_client: Taxonomy, category_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        if not df.index.is_unique:
            logger.warning(f"Duplicate indices found in '{category_name}' taxonomy. Keeping first.")
...
def get_company_code_mapping(taxonomy_client: Taxonomy) -> Dict[str, str]:
    ...
    Returns:
        Dictionary mapping uppercase company codes to company names
    """
    df = get_category_codes(taxonomy_client, "companies")
    if not df.empty:
        # Ensure codes are uppercase for consistent matching if needed later
